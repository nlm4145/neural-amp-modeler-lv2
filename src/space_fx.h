...
class DelayLine {
 public:
  ...
  float read(size_t delay) const {
    delay = std::min(std::max<size_t>(delay, 1), capacity());
    const size_t n = buffer_.size();
    return buffer_[(write_ + n - delay) % n];
  }
  float readFrac(double delay) const {
    delay = std::min(std::max(delay, 1.0), static_cast<double>(capacity()));
 ...
 public:
  ...
  void process(float* left, float* right, size_t count, float delayMs) {
    ...
    for (size_t i = 0; i < count; ++i) {
      ...
      right_.write(right[i]);
      if (smoothed_ < 1.0) {
        const float mixL = left_.read(1), mixR = right_.read(1);
        left[i] += static_cast<float>(smoothed_) * (mixL - left[i]);
        right[i] += static_cast<float>(smoothed_) * (mixR - right[i]);
      } else {
 ...
 public:
  ...
  void process(float* left, float* right, size_t count, float timeMs,
    ...
    for (size_t i = 0; i < count; ++i) {
      ...
      const float tapL = left_.readFrac(smoothedTime_);
      const float tapR = right_.readFrac(smoothedTime_);
